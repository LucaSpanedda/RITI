...
In fixed version values similiar to values already saved in a range of 40Hz are deletted, Out in filtered.lib
''')
...
# Initialize a list to store the indices of the elements to delete
to_delete = []

# Iterate over the frequencies and amplitudes and find elements that are within 10 of each other
for i in range(len(highest_frequencies)):
    for j in range(i+1, len(highest_frequencies)):
        if abs(highest_frequencies[i] - highest_frequencies[j]) < 40:
            to_delete.append(j)

# Create a Boolean mask to select the elements to keep
mask = np.ones(len(highest_frequencies), dtype=bool)
mask[to_delete] = False

# Use the mask to filter the frequencies and amplitudes lists
filtered_frequencies = highest_frequencies[mask]
filtered_amplitudes = highest_amplitudes[mask]

# Save the selected frequencies
with open((root)+"filtered.lib", "w") as f:
    f.write((root)+"_filteredfrequencies = (\n")
    for i in range(len(filtered_frequencies)):
        if i == len(filtered_frequencies) - 1:
            f.write(str(filtered_frequencies[i]) + ") ; \n \n")
        else:
            f.write(str(filtered_frequencies[i]) + ", ")

# Save the scaled amplitudes
    f.write((root)+"_filteredamplitudes = (\n")
    for i in range(len(filtered_amplitudes)):
        if i == len(filtered_amplitudes) - 1:
            f.write(str(filtered_amplitudes[i]) + ") ; \n \n")
        else:
            f.write(str(filtered_amplitudes[i]) + ", ")
...
print(len(filtered_frequencies))