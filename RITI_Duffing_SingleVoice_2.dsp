...
// GUI
gui = si.smoo(vslider("bandwidth", 30, 1, 50, 0.0001)), si.smoo(vslider("frequency", 0, -1, 1, 0.0001)), si.smoo(vslider("smooth", 1, 0, 1, 0.0001)), si.smoo(vslider("gain", 1, 0, 1, 0.0001)), si.smoo(vslider("damping", 0.3, 0, 1, 0.0001)), si.smoo(vslider("mod", 0, 0, 1, 0.0001)), si.smoo(vslider("rate", 62, 5, 200, 0.0001)), _;

// Duffing Equation
duffing(bandwidth, freqshift, smooth, gain, damping, mod, rate, x) = (yn : (_, filtered)) ~ si.bus(2)
    with{
        filtered = _ * gain <: (BPBank(24, 1, bandwidth, 16 ^ freqshift) <: si.bus(2)), _ : (_, ro.cross(2)) : (atan(_ * 1.0 + _ * 1.0) : fi.lowpass(2, smooth * 9990 + 10, _)), _;
        c0 = max(.0001, min(1,  1 - damping));
        c = (c0 * c0 * c0 + 0.00001) : si.smoo;
        gamma = 1 - mod;
        omega =  rate;
        dt = (omega / ma.SR : + ~ _ % (60 * omega)) + x;
        yn(y, x) = (x - (x * x * x))' - (c * y) + gamma * sin(dt) <: si.bus(2);
    };
duffingout = duffing : (!, !, _ * 0.75);
process = hgroup("voice_0", _ * 0.00 : gui : duffingout) <: si.bus(2);
