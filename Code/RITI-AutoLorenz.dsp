...
Bandwidth = hslider("Bandwidth",3,.1,10,.001) : si.smoo;
SingleUnitInternalFBGain = hslider("SingleUnitInternalFBGain", 1, 0, 1, .001): si.smoo;
Ktf = hslider("tahn", 10, 1, 10, .001) : si.smoo;
...
// Luca Spanedda - Perturbations I - Installation Code

// Autoregulating Lorenz System
autolorenzL(in,dcfc,l) = loop ~ si.bus(7): par(i, 7, /(l)): mixer
    with {
        saturator(lim,x) = lim*ma.tanh(x);
        dcblock(dcfc,x) = fi.highpass(1, dcfc, x);
        loop(x, y, z, sigma, dt, rho, beta) = 
            fi.notchw(20,2409, filterbanks(BPFilterOrder, 64, 10, saturator(l, dcblock(10,((x+in) + sigma * ((y+in) - (x+in)) * dt))) ) ), 
            fi.notchw(20,2409, filterbanks(BPFilterOrder, 64, 10, saturator(l, dcblock(10,((y+in) + (rho * (x+in) - (x+in) * z - (y+in)) * dt ))) ) ),  
            fi.notchw(20,2409, filterbanks(BPFilterOrder, 64, 10, saturator(l, dcblock(10,((z+in) + ((x+in) * (y+in) - beta * (z+in)) * dt ))) ) ),   
    ...
    autolorenzR(in,dcfc,l) = loop ~ si.bus(7): par(i, 7, /(l)): mixer
    with {
        saturator(lim,x) = lim*ma.tanh(x);
        dcblock(dcfc,x) = fi.highpass(1, dcfc, x);
        loop(x, y, z, sigma, dt, rho, beta) = 
